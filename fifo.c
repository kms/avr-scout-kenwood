...
inline uint8_t isFifoFull(fifo *c) {
    return ((c->write + 1) % c->capacity) == c->read;
}

inline uint8_t isFifoEmpty(fifo *c) {
    return c->write == c->read;
} 

inline uint8_t fifoSize(fifo *c) {
    return c->size;
} 

inline uint8_t fifoGet(fifo *c) {
    c->size--;
    c->read = (c->read + 1) % c->capacity;
    return c->buffer[c->read];
}       

inline void fifoPut(fifo *c, uint8_t p) {
    c->size++;
